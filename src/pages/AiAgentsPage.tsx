...
export default function AiAgentsPage() {
  ...
  return (
    <div className="min-h-screen bg-[hsl(210_20%_99%)] font-sans text-zinc-950 antialiased">
      ...
      {/* ── Hero ── */}
      <header className="relative overflow-hidden border-b border-zinc-800 bg-black" style={{ minHeight: "min(44vh, 460px)" }}>
        ...
        <div className="relative z-10 mx-auto flex max-w-5xl flex-col gap-6 px-4 pb-8 pt-8 sm:gap-7 sm:px-6 sm:pb-10 sm:pt-10">
          <div className="flex items-center justify-between gap-4">
            <Link to="/" aria-label="Vekta home" className="inline-flex shrink-0 items-center outline-none transition-opacity hover:opacity-90">
              <img
                src="/brand/vekta-hero-wordmark.svg"
                alt=""
                width={120}
                height={32}
                className="h-7 w-auto max-w-[min(40vw,9rem)] object-contain object-left sm:h-8 sm:max-w-[10rem]"
                decoding="async"
              />
            </Link>
            <DropdownMenu>
              <DropdownMenuTrigger
                type="button"
                className="inline-flex shrink-0 items-center gap-1.5 rounded-md px-2 py-1.5 text-sm font-medium text-[#eeeeee]/80 outline-none transition-colors hover:bg-white/[0.06] hover:text-[#eeeeee] data-[state=open]:bg-white/[0.06] data-[state=open]:text-[#eeeeee]"
              >
          ...
          <div className="max-w-2xl space-y-4">
            ...
            <div className="flex flex-wrap items-center gap-3">
              <Button
                ...
                onClick={() => document.getElementById("agents-grid")?.scrollIntoView({ behavior: "smooth", block: "start" })}
              >
                Explore agents
              </Button>
              <Button
      ...
      {betaAgents.length > 0 && (
        <section id="agents-grid" className="bg-white">
          <div className="mx-auto max-w-5xl px-4 py-12 sm:px-6 sm:py-16">
            <div className="mb-2 inline-flex items-center gap-2 rounded-full border border-violet-500/30 bg-violet-50 px-3 py-1">
      ...
      <footer className="border-t border-zinc-200/80 bg-white py-8 text-center text-xs text-zinc-500">
        <p>
          ...
          <Link to="/access" className="font-medium text-zinc-700 underline-offset-2 hover:underline">
            Create an account
          </Link>{" "}
          ·{" "}
