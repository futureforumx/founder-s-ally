...
export function resolveWorkOSApiHostname(): string | undefined {
  const explicit = trim(import.meta.env.VITE_WORKOS_API_HOSTNAME);
...
export function resolveWorkOSDevMode(): boolean {
  ...
  // WorkOS recommends devMode for client-only apps that still use the default
  // hosted auth domain instead of a custom auth domain.
  return !resolveWorkOSApiHostname();
}
