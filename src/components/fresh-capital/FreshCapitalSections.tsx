  ...
  return (
    <section className="bg-white">
      <div className="mx-auto max-w-5xl px-4 py-12 sm:px-6 sm:py-16">
        <h2 className="text-lg font-semibold tracking-tight text-zinc-950">Why this matters</h2>
        <p className="mt-4 max-w-2xl text-pretty text-base leading-relaxed text-zinc-600">
          Investors are most responsive right after raising a new fund. Vekta surfaces fresh capital in real time—so you reach the right investors when they’re actively writing checks.
        </p>
      </div>
    </section>
  );
    ...
    <section className="border-y border-zinc-200/80 bg-zinc-50/50">
      <div className="mx-auto max-w-5xl px-4 py-12 sm:px-6 sm:py-16">
        <div className="mb-6 flex items-start justify-between gap-4">
          <div>
            <h2 className="text-lg font-semibold tracking-tight text-zinc-950">Inside Vekta</h2>
            <p className="mt-1 max-w-xl text-sm text-zinc-600">
              Full access turns announcements into action—who is deploying, where they lean, and when to move.
            </p>
          </div>
          <Lock className="mt-1 h-5 w-5 shrink-0 text-zinc-400" aria-hidden />
        </div>

        <div className="relative grid gap-4 md:grid-cols-2">
          <div
            className="relative overflow-hidden rounded-2xl border border-zinc-200 bg-white p-5 shadow-sm outline-none ring-offset-2 focus-visible:ring-2 focus-visible:ring-violet-500"
            role="button"
            tabIndex={0}
          ...
          >
            <p className="text-xs font-semibold uppercase tracking-wide text-violet-600">Example</p>
            <p className="mt-2 font-medium text-zinc-950">Aurora Stack Capital — $300M Fund III</p>
            ...
            <div
              className="pointer-events-none absolute inset-0 flex items-end justify-center bg-gradient-to-t from-white/95 via-white/40 to-transparent pb-4"
              aria-hidden
            >
          ...
          >
            <p className="text-xs font-semibold uppercase tracking-wide text-violet-600">Example</p>
            <p className="mt-2 font-medium text-zinc-950">Coastal Robotics Ventures — Fund II</p>
            <ul className="mt-3 space-y-2 text-sm text-zinc-700">
            ...
            <div className="pointer-events-none absolute bottom-4 left-5 right-5 flex justify-center">
              <span className="inline-flex items-center gap-1.5 rounded-full bg-white/95 px-3 py-1 text-2xs font-medium text-zinc-700 shadow ring-1 ring-zinc-200">
                <Lock className="h-3 w-3" aria-hidden />
                Blurred in preview
              </span>
            </div>
...
export function FreshCapitalHeatmap({ buckets }: { buckets: HeatmapBucket[] }) {
  return (
    <section className="bg-white">
      <div className="mx-auto max-w-5xl px-4 py-12 sm:px-6 sm:py-16">
        <h2 className="text-lg font-semibold tracking-tight text-zinc-950">Capital activity by sector</h2>
        <p className="mt-1 text-sm text-zinc-600">Where new fund announcements have clustered recently.</p>
            ...
            {buckets.map((b) => (
              <li key={b.label} className="flex flex-col gap-1.5 sm:flex-row sm:items-center sm:gap-4">
                <div className="flex w-full max-w-[200px] items-center justify-between gap-2 sm:justify-start">
                  <span className="text-sm font-medium text-zinc-900">{b.label}</span>
                  <span
                    className={cn(
                      "rounded-full px-2 py-0.5 text-2xs font-medium",
                      b.tier === "high" && "bg-emerald-100 text-emerald-900",
                      b.tier === "moderate" && "bg-amber-100 text-amber-900",
                      b.tier === "low" && "bg-zinc-100 text-zinc-700",
                    )}
                  >
                    {tierLabel(b.tier)}
                  </span>
  ...
  return (
    <section className="border-t border-zinc-200/80 bg-zinc-950 text-white">
      <div className="mx-auto flex max-w-5xl flex-col items-start gap-6 px-4 py-14 sm:flex-row sm:items-center sm:justify-between sm:px-6 sm:py-16">
        ...
        <Button asChild size="lg" className="rounded-full bg-white text-zinc-950 hover:bg-zinc-100">
          <Link
            to={signupHref}
            onClick={() => {
