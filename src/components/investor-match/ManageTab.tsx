...
import { Users, Plus, Search, Settings2, DollarSign, Pencil, Check, X, UserPlus } from "lucide-react";
...
function useNFXSearch(query: string) {
  ...
  useEffect(() => {
    ...
    debounceRef.current = setTimeout(() => {
      // Simulated NFX Signal results — replace with real Apify call
      const mock: NFXResult[] = [
        { name: "Sequoia Capital", location: "Menlo Park, CA", logoUrl: "" },
        { name: "Andreessen Horowitz", location: "Menlo Park, CA", logoUrl: "" },
        { name: "Y Combinator", location: "San Francisco, CA", logoUrl: "" },
        { name: "Benchmark Capital", location: "San Francisco, CA", logoUrl: "" },
        { name: "Greylock Partners", location: "Menlo Park, CA", logoUrl: "" },
        { name: "Accel Partners", location: "Palo Alto, CA", logoUrl: "" },
        { name: "Lightspeed Venture Partners", location: "Menlo Park, CA", logoUrl: "" },
        { name: "Founders Fund", location: "San Francisco, CA", logoUrl: "" },
...
function CapTablePanel({ confirmedBackers, formatCurrency }: Omit<ManageTabProps, "totalRaised">) {
  ...
  const { results: nfxResults, loading: nfxLoading } = useNFXSearch(searchQuery);
    ...
    >
      ...
      <div ref={searchRef} className="relative mb-6">
        ...
        {showSuggestions && searchQuery.length >= 2 && (
          ...
          >
            <div className="px-4 py-2.5 border-b" style={{ borderColor: "hsla(var(--border), 0.5)" }}>
                ...
                {nfxResults.map((r, i) => (
                  <button
                    ...
                    onClick={() => {
                      setSearchQuery("");
                      setShowSuggestions(false);
                      // In production: auto-add to cap table
                    }}
  ...
  return (
    <div className="space-y-5">
      ...
      >
        ...
        <div className="space-y-5">
          <div>
            <label className="text-[10px] font-semibold text-muted-foreground uppercase tracking-[0.15em]">
              Target Raise
            </label>
            <p
          ...
          <div>
            ...
            <div className="mt-1.5">
              <Badge
                ...
                style={{ background: "hsla(var(--accent), 0.1)", color: "hsl(var(--accent))" }}
              >
                {roundStage}
              </Badge>
            </div>
        ...
        <div className="space-y-3.5">
          {[
            ...
            <div key={item.label} className="flex items-center justify-between">
              ...
              <span className="text-xs font-semibold text-foreground" style={{ fontFamily: "'Geist Mono', monospace" }}>
                {item.value}
              </span>
            </div>
          ))}
