...
export function StageTimeline({
  ...
  return (
    <div className={`rounded-xl border border-border bg-card p-3 ${isExpanded ? "h-[calc(100vh-280px)] max-h-[520px] flex flex-col justify-center" : "h-full"}`}>
                ...
                >
                  <div
                    className={`relative z-10 rounded-full border-2 transition-all ${
                      isSweetSpot
                        ? `h-3.5 w-3.5 bg-success border-success shadow-[0_0_8px_hsl(var(--success)/0.4)] ${isHovered ? "scale-125" : ""}`
                        : isInvestArea
                          ? `h-3 w-3 bg-accent border-accent shadow-[0_0_6px_hsl(var(--accent)/0.3)] ${isHovered ? "scale-125" : ""}`
                          : `h-2 w-2 bg-muted border-border ${isHovered ? "scale-110" : ""}`
                    }`}
                  />
                  <span className={`text-[9px] font-medium whitespace-nowrap transition-colors ${
                    isSweetSpot ? "text-success font-semibold" : isInvestArea ? "text-foreground" : "text-muted-foreground"
                  }`}>
                    {stage}
                  </span>
                </div>
