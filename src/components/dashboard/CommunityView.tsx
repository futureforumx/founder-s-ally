...
export function CommunityView({
  ...
  const bestMatchesSubtitle = "Curated matches based on your profile";
  ...
  const handleOperatorPreviewClick = useCallback(
    (inv: InvestorPreviewModel) => {
      ...
      const entry =
        mergedEntries.find((e) => e.name === inv.name && e.category !== "investor") ??
        pool.find((e) => e.name === inv.name);
    ...
    <div className="space-y-2">
      ...
      {showInvestorRails || showNetworkRails || showOperatorDirectoryRails ? (
        <>
          {showInvestorRails && (
            <div className="pt-4">
              <InvestorSuggestedTrendingRails
                ...
                suggestedSubtitle={bestMatchesSubtitle}
          ...
          {showOperatorDirectoryRails && (
            <div className="pt-4">
              <InvestorSuggestedTrendingRails
                rowKind="operator"
                suggested={operatorDirectoryRailSuggestedRanked.map((e) => directoryEntryToOperatorPreview(e))}
                trending={operatorDirectoryRailTrending.map((e) => directoryEntryToOperatorPreview(e))}
                suggestedTitle={operatorDirectoryRailTitles.suggested}
                suggestedSubtitle={operatorDirectorySuggestedSubtitle}
                trendingTitle={operatorDirectoryRailTitles.trending}
                trendingSubtitle="Most active this week"
                onViewAllSuggested={handleViewAll}
                onViewAllTrending={handleViewAll}
                onPreviewClick={handleOperatorPreviewClick}
                anchorVcFirmId={() => null}
              />
            </div>
          )}
