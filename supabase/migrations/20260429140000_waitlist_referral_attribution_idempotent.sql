-- Idempotent schema guards + referral attribution hardening (no double credit on conflict).

ALTER TABLE public.waitlist_users
  ADD COLUMN IF NOT EXISTS referred_by_user_id uuid REFERENCES public.waitlist_users(id) ON DELETE SET NULL;

ALTER TABLE public.waitlist_users
  ADD COLUMN IF NOT EXISTS referral_count integer NOT NULL DEFAULT 0;

CREATE INDEX IF NOT EXISTS idx_waitlist_users_referred_by
