-- Make the Latest Funding company mark explicitly editable from the admin console.
-- The public feed still falls back to website-derived favicons when this is null.

ALTER TABLE public.fi_deals_canonical
  ADD COLUMN IF NOT EXISTS company_logo_url text;

COMMENT ON COLUMN public.fi_deals_canonical.company_logo_url IS
  'Optional explicit logo/mark URL for Fresh Capital Latest Funding rows; falls back to company_website favicon.';
...
AS $body$
  ...
  SELECT
    id, company_name, website_url, company_logo_url, sector, round_kind, amount_label,
...
$fn$;
    ...
    COMMENT ON FUNCTION public.get_recent_funding_feed(integer) IS
      'Union funding_deals + fi_deals_canonical for Fresh Capital Latest Funding; includes explicit company_logo_url for admin-editable company marks.';

    GRANT EXECUTE ON FUNCTION public.get_recent_funding_feed(integer)
      TO anon, authenticated, service_role;
  ELSE
    RAISE NOTICE 'Funding feed tables missing; company_logo_url column was added but get_recent_funding_feed was not redefined.';
  END IF;
END
$migration$;
