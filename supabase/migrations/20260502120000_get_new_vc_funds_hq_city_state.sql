-- firm_location: HQ only — prefer City, ST from hq_city + hq_state (not fund geography_focus).
...
COMMENT ON FUNCTION public.get_new_vc_funds(integer, integer, text[], text[], text[], numeric, numeric, text[]) IS
  'Public-safe VC fund rows; firm_location = HQ (City, ST from hq_city + hq_state), not fund geography_focus.';
