-- Add likely_actively_deploying to get_new_vc_funds so public Fresh Capital can badge rows
-- using the same boolean logic as get_active_funds_by_stage, without a second RPC merge.
...
COMMENT ON FUNCTION public.get_new_vc_funds(integer, integer, text[], text[], text[], numeric, numeric, text[]) IS
  'Public-safe recent VC fund announcements; includes likely_actively_deploying (same rule as get_active_funds_by_stage).';
