-- Remote drift: do not assume prior migrations ran. Ensures columns + index, then reapplies waitlist_signup.

-- migration: ensure referral attribution columns exist
ALTER TABLE public.waitlist_users
  ADD COLUMN IF NOT EXISTS referred_by_user_id uuid
  REFERENCES public.waitlist_users(id)
  ON DELETE SET NULL;
