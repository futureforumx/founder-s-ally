-- Expose firm logo + domain on public.get_new_vc_funds for Fresh Capital firm marks (logo or favicon).
...
COMMENT ON FUNCTION public.get_new_vc_funds(integer, integer, text[], text[], text[], numeric, numeric, text[]) IS
  'Public-safe recent VC fund announcements; includes likely_actively_deploying + firm_logo_url/firm_domain for Fresh Capital UI.';
