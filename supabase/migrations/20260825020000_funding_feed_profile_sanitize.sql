-- Allow unicode HQ lines (e.g. São Paulo) and drop AlleyWatch/tweet dump descriptions.
    ...
    SELECT
      ...
      CASE
        ...
        WHEN char_length(btrim(fd.company_hq)) BETWEEN 3 AND 80
          AND btrim(fd.company_hq) ~ '^[[:alpha:]0-9 .''-]+,[[:space:]]*[[:alpha:]0-9 .''-]+$'
          AND btrim(fd.company_hq) !~* 'raises|http|toggle|subscribe'
      ...
      CASE
        ...
        WHEN char_length(btrim(fd.deal_summary)) BETWEEN 40 AND 900
          AND btrim(fd.deal_summary) !~* 'join for free|toggle search|flash sale|subscribe sign in|alleywatch|share on facebook|no result view all|on x:'
    ...
    SELECT
      ...
      CASE
        ...
        WHEN char_length(btrim(fdc.company_location)) BETWEEN 3 AND 80
          AND btrim(fdc.company_location) ~ '^[[:alpha:]0-9 .''-]+,[[:space:]]*[[:alpha:]0-9 .''-]+$'
          AND btrim(fdc.company_location) !~* 'raises|http|toggle|subscribe'
      ...
      CASE
        ...
        WHEN char_length(btrim(fdc.extracted_summary)) BETWEEN 40 AND 900
          AND btrim(fdc.extracted_summary) !~* 'join for free|toggle search|flash sale|subscribe sign in|alleywatch|share on facebook|no result view all|on x:'
