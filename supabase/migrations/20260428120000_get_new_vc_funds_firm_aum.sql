-- Expose firm-level AUM from firm_records.aum for Fresh Capital meta row.
-- Fund vehicle size remains in target_size_usd / final_size_usd on vc_funds.
...
AS $$
  ...
  SELECT
    ...
    CASE
      WHEN fr.aum IS NOT NULL AND fr.aum::numeric > 0 THEN fr.aum::numeric
      ELSE NULL
    END AS firm_aum_usd
...
COMMENT ON FUNCTION public.get_new_vc_funds(integer, integer, text[], text[], text[], numeric, numeric, text[]) IS
  'Public-safe recent VC fund announcements; firm mark, HQ, website, firm AUM (firm_records.aum), plus fund fields.';
