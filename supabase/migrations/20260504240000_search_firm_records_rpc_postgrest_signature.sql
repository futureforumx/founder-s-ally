-- PostgREST binds RPC JSON args using alphabetical key order → positional parameters:
-- { p_limit, p_query, p_ready_for_live } → (integer, text, boolean).
-- The original overload public.search_firm_records(text, integer, boolean) does not match that order,
-- which yields: "Could not find the function ... (p_limit, p_query, p_ready_for_live) in the schema cache".
...
GRANT EXECUTE ON FUNCTION public.search_firm_records(integer, text, boolean) TO anon, authenticated, service_role;
