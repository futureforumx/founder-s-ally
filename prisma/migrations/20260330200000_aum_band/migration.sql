...
-- AlterTable
ALTER TABLE "vc_firms" ADD COLUMN "aum_band" "AumBand";
ALTER TABLE "vc_funds" ADD COLUMN "aum_band" "AumBand";

-- CreateIndex
CREATE INDEX "vc_firms_aum_band_idx" ON "vc_firms"("aum_band");
CREATE INDEX "vc_funds_aum_band_idx" ON "vc_funds"("aum_band");

-- Backfill fund-level band from larger of AUM vs fund size (USD)
UPDATE "vc_funds"
SET "aum_band" = CASE
...
-- Firm-level band: largest vehicle among active funds (flagship-style rollup)
UPDATE "vc_firms" f
SET "aum_band" = x.b
FROM (
  SELECT "firm_id",
