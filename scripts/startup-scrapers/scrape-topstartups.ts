...
async function scrape(): Promise<void> {
  ...
  try {
      ...
      const embeddedData = await page.evaluate(() => {
        ...
        for (const script of scripts) {
          ...
          try {
            ...
            if (Array.isArray(parsed) && parsed.length > 2 && (parsed[0]?.name || parsed[0]?.title)) return parsed;
          } catch { /* not valid JSON */ }
        }
        return null;
      });
      if (embeddedData) {
        console.log(`[topstartups] Got ${embeddedData.length} companies from embedded JSON`);
    ...
    if (allCompanies.length === 0) {
      ...
      console.log(`[topstartups] WARNING: 0 companies found — diagnostics:`);
