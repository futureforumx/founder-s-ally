...
function selectAiMarkdownWindow(markdown: string): string {
  const lines = markdown.split(/\r?\n/);
  if (lines.length < 40) return markdown.slice(0, 60_000);

  const sectionHeaderRe = /^\s*#{1,6}\s*(the\s+a\s*-?\s*team|team|people|partners?|leadership|investment\s+team|our\s+team)\b/i;
  const personHeadingRe = /^\s*#{2,6}\s+[A-Z][A-Za-z'.-]+(?:\s+[A-Z][A-Za-z'.-]+){1,3}\s*$/;

  const sectionIdx = lines.findIndex((line) => sectionHeaderRe.test(line));
  const personIdx = lines.findIndex((line) => personHeadingRe.test(line));
  const start = sectionIdx >= 0 ? sectionIdx : personIdx >= 0 ? Math.max(0, personIdx - 4) : -1;
  if (start < 0) return markdown.slice(0, 60_000);

  let end = lines.length;
  for (let i = Math.min(lines.length - 1, start + 24); i < lines.length; i += 1) {
    const line = lines[i];
    if (/^\s*#{1,3}\s+/.test(line) && !sectionHeaderRe.test(line)) {
      end = i;
      break;
    }
  }

  const window = lines.slice(Math.max(0, start - 2), Math.min(lines.length, end + 2)).join("\n").trim();
  if (window.length < 300) return markdown.slice(0, 60_000);
  return window.slice(0, 35_000);
}
...
async function main() {
  ...
  for (let i = 0; i < firms.length; i += 1) {
    ...
    try {
      ...
      const aiMarkdownCandidates: Array<{ url: string; markdown: string }> = [];
      ...
      for (const pageUrl of candidatePages) {
        ...
        if (!page) continue;
        if (page.markdown && page.markdown.trim().length > 120) {
          aiMarkdownCandidates.push({ url: page.url, markdown: page.markdown });
        }
      ...
      console.log(
        `[team-sync]   extraction counts ai=${aiPeople.length} heuristicRaw=${extractedRaw.length} heuristicMerged=${extracted.length}`,
      );
